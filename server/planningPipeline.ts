 ...
 */
export const SHARED_PLANNING_RULES = `CRITICAL RULE - BRING REAL EXPERTISE, NEVER GENERIC PLACEHOLDER CONTENT:
...
- Ground timing in real-world logistical knowledge, not arbitrary round numbers: private entertainment/experience bookings typically need 3-4 weeks for weekend slots, high-demand restaurants 2-3 weeks, flights/lodging 4-6 weeks, custom/monogrammed goods 2-3 weeks for production, timed museum/attraction tickets often release on a fixed weekly schedule weeks ahead, bakeries/custom cakes 5-7 days out, 24-hour airline check-in, day-of travel buffers 1-2 hours.
- Every milestone's "description" must state a SPECIFIC reason grounded in that real knowledge, tailored to this exact event - never a generic placeholder like "prepare in advance" or "secure this before it's too late" that could be pasted onto any milestone in any event unchanged.
- Plan for what happens AFTER the event starts or ends when a real obligation exists there, not only before it. Use a positive offset (e.g. t_minus_days: -1 for "Day +1") for these. Examples: a scuba trip's no-fly window (final dives at least 18-24 hours before any flight home, per DAN/PADI guidance), a return-flight check, trip expense filing, a post-launch review, recovery/cooldown after a procedure. Never skip a safety-relevant post-event milestone just to keep the list short.
- This does not relax the "specific, concrete thing" or "1-3 deliverables" rules elsewhere in these instructions - it raises the bar on what fills them. A concrete title paired with a generic, could-apply-to-anything description has not actually used your knowledge.
...
- Only drop or rewrite an existing milestone if the new message explicitly contradicts it. A destination swap is one case ("actually we're not going to Paris anymore, going to Rome instead" replaces the destination-specific tasks). A transport-mode swap is exactly the same kind of contradiction and just as common: "no flights, we're going by train instead" means REPLACE every flight-specific milestone/deliverable with train-specific ones - it does NOT mean adding a train task alongside the existing flight one, and it does NOT mean a milestone title can still mention "flights" anywhere (e.g. "Flights, trains & hotel reservation lock" is wrong - it should read "Trains & hotel reservation lock" or similar, with every trace of the ruled-out mode gone). By contrast, "we also need a dog sitter" does not touch anything else on the trip.
- "MERGE" applies to the milestone list, not to staying silent about the event's own date/location: if the new message states an explicit date or location, output that as the event's own top-level date/location fields too (whatever your output schema calls them), even when merging into an existing plan - never omit these fields just because they were "already set" before, and never let an old date survive when the user just gave a new one. The event's title/name is handled separately and outside your control on a merge turn - don't worry about that field here.
...
CRITICAL RULE - FLAG is_open_decision ONLY FOR A KEY ESSENTIAL, NEVER FOR AN ORDINARY TASK:
is_open_decision is for a small handful of load-bearing, foundational facts that the rest of the plan genuinely depends on and that a Yes/No or a 2-3 option pick can resolve outright - the kind of thing that changes WHAT gets planned, not how one task gets carried out. The doc's own worked examples: is the user hosting or attending (Yes/No), where is this happening (home or a venue), is a gift needed and how (group/solo/none), is a cake or catering needed at all (Yes/No), are there other logistics/extras needed (Yes/No). That is the ENTIRE scope - do not invent categories beyond it.
This is NOT for an ordinary actionable task that already has a clear objective, even if a small specific hasn't been picked yet - "place the cake order for Sunday pickup," "confirm the RSVP headcount," "book the venue" are normal gettable-on-with tasks, not open decisions, even though each involves a choice of provider/time/etc. A milestone or deliverable already has a status and a deadline; that alone never qualifies it. Ask yourself: does the REST OF THE PLAN change shape depending on the answer? If the answer is a specific vendor, date, or logistics detail rather than a fork in what needs to happen at all, it is not an open decision - never set is_open_decision for it, and never invent a decision just to have something to ask. Most events will have ZERO open decisions once these key essentials are established - that is the expected, correct outcome, not a sign something was missed.
When a key essential genuinely IS still unknown, set that specific milestone or deliverable's is_open_decision to true with 2-3 concrete decision_options (or exactly Yes/No when that is the actual shape of the choice) - never a generic placeholder option. Unlike the ONE proactive follow-up above (a NEW thing the plan doesn't yet cover), an open decision is an EXISTING item stuck until the user picks - flag every genuinely open key essential, not just one, and keep it flagged on every future turn until actually resolved, never silently dropped just because this turn didn't mention it.`;

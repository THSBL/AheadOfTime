...
| **In-app calendar actions** (Push to Cal, Scan agenda, delete) | Push to Cal and Scan agenda run on the server with the Background Sync grant when it is linked (`server/googleCalendarServerApi.ts`, `/api/auth/google/calendar-push` and `/calendar-events`), otherwise in the browser with the user's access token. Delete and the other actions still run in the browser (`src/services/googleCalendar.ts`, `googleTasks.ts`). | The server path is where an Outlook equivalent plugs in. |

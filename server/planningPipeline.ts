...
CRITICAL RULE - FLAG A MILESTONE OR DELIVERABLE AS is_open_decision WHEN IT GENUINELY IS ONE:
Some items in the plan are not yet actionable because a real choice hasn't been made - "home dinner or a restaurant reservation," "solo gift or a group pooled gift." When one of these is genuinely still open (not just a detail you could reasonably assume), set that specific milestone or deliverable's is_open_decision to true and give it 2-3 concrete decision_options - never a generic placeholder option, and never flag something that's actually a normal, gettable-on-with task. This is different from the ONE proactive follow-up above (that's a NEW thing the plan doesn't cover yet; this is an EXISTING item in the plan that's stuck until the user picks). Unlike the one-per-turn follow-up, flag every genuinely open item, not just one - each stays flagged, visible, and re-askable on every future turn until the user actually resolves it (by picking an option or stating a preference), never silently dropped just because this turn didn't mention it.`;

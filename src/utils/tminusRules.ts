...
export function generateICSContent(event: CalendarEvent): string {
  ...
  let ics = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//T-Minus Calendar Intelligence Agent//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    `X-WR-CALNAME:${event.title} + T-Minus Schedule`,
  ];
  ...
  ics.push(`UID:main-${event.id}@tminus.agent`);
  ...
  ics.push(`DESCRIPTION:Event prepared with T-Minus Calendar Intelligence Agent.\\nStatus: ${event.status}\\nCategory: ${event.category}`);
  ...
  event.milestones.forEach((ms) => {
    ...
    ics.push('BEGIN:VEVENT');
    ics.push(`UID:${ms.id}@tminus.agent`);
    ics.push(`DTSTAMP:${nowICS}`);
    ...
    ics.push(`DESCRIPTION:T-Minus Milestone for ${event.title}\\nCategory: ${ms.category}\\nDetail: ${ms.description || 'Milestone action'}`);
    ics.push('STATUS:CONFIRMED');
...
export function formatMessagingSummary(event: CalendarEvent): string {
  ...
  lines.push('');
  lines.push(`_Generated by T-Minus Preparation Agent_`);
  return lines.join('\n');
}

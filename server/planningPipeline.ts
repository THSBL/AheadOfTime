...
- "MERGE" applies to the milestone list, not to staying silent about the event's own date/location: if the new message states an explicit date or location, output that as the top-level target_date/location too, even when merging into an existing plan - never omit these fields just because they were "already set" before, and never let an old date survive when the user just gave a new one. The event's title/name is handled separately and outside your control on a merge turn - don't worry about event_title here.
